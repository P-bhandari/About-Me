...
const projectSchema = z.object({ title: text(160), summary: text(1200), tags: text(400), repoUrl: nullableUrl, liveUrl: nullableUrl, imageKey: nullableText(500), featured: z.boolean().default(true), published: z.boolean(), archived: z.boolean().default(false), sortOrder: z.number().int().min(0).max(999) });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (command.action === 'delete') {
      ...
      if (command.action === 'create') await db.prepare(`INSERT INTO projects (title,summary,tags,repo_url,live_url,image_key,featured,published,archived,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?)`).bind(v.title,v.summary,v.tags,v.repoUrl,v.liveUrl,v.imageKey,v.featured?1:0,v.published?1:0,v.archived?1:0,v.sortOrder).run();
      else { if (!command.id) throw new Error('Project id is required'); await db.prepare(`UPDATE projects SET title=?,summary=?,tags=?,repo_url=?,live_url=?,image_key=?,featured=?,published=?,archived=?,sort_order=? WHERE id=?`).bind(v.title,v.summary,v.tags,v.repoUrl,v.liveUrl,v.imageKey,v.featured?1:0,v.published?1:0,v.archived?1:0,v.sortOrder,command.id).run(); }
